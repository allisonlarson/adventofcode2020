...
func TestComputePart1(t *testing.T) {
	rawInput := `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
func TestComputePart2(t *testing.T) {
	rawInput := `shiny gold bags contain 2 dark red bags.
...
`
	...
	if answer != 126 {
		t.Errorf("Expected 126, got %v", answer)
	}
	rawInput = `light red bags contain 1 bright white bag, 2 muted yellow bags.
dark orange bags contain 3 bright white bags, 4 muted yellow bags.
bright white bags contain 1 shiny gold bag.
...
`
	...
	if answer != 32 {
		t.Errorf("Expected 32, got %v", answer)
	}
}
